...
import { Building2, Sparkles, Clock, TrendingUp, Users, MessageSquare, CircleCheck as CheckCircle2, ChevronDown, ArrowRight, CirclePlus as PlusCircle, Zap, Menu, X } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#050505] text-[#F5F5F5] font-sans overflow-x-hidden">
      ...
      <nav className="fixed top-0 w-full z-50 bg-[#050505]/80 backdrop-blur-md border-b border-[#1A1A1A]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          ...
          <div className="hidden sm:flex items-center gap-4 text-[9px] uppercase tracking-widest font-bold text-[#888]">
            <button onClick={onPricing} className="hover:text-gold transition-colors">Pricing</button>
            <button onClick={onLogin} className="luxury-button px-4 py-1.5 text-[9px]">Sign In</button>
          </div>
          <button
            onClick={() => setMobileNavOpen(!mobileNavOpen)}
            className="sm:hidden p-2 rounded-lg text-[#888] hover:text-white hover:bg-[#1A1A1A] transition-all"
          >
        ...
        {/* Mobile Nav Dropdown */}
        {mobileNavOpen && (
          <div className="sm:hidden border-t border-[#1A1A1A] bg-[#050505]/95 backdrop-blur-xl px-4 py-4 space-y-3">
            <button onClick={onPricing} className="block w-full text-left text-sm text-[#888] hover:text-gold transition-colors py-2">Pricing</button>
            <button onClick={onLogin} className="luxury-button w-full py-2.5 text-[10px]">Sign In</button>
          </div>
        )}
      </nav>

      {/* Hero Section */}
      <section className="relative min-h-[calc(100vh-3.5rem)] flex items-center pt-24 pb-16 sm:pt-20 sm:pb-20 px-4 sm:px-6 overflow-hidden">
        <div className="max-w-4xl mx-auto w-full relative z-10">
          <div className="flex flex-col items-center text-center space-y-6 sm:space-y-8 lg:space-y-10">
            <motion.h1
              ...
              className="text-3xl sm:text-4xl lg:text-7xl font-sans font-medium leading-[1.1] text-white tracking-tight"
            >
            ...
            <motion.p
              ...
              className="text-sm sm:text-base lg:text-lg text-[#888] max-w-xl leading-relaxed px-2"
            >
            ...
            >
              <button onClick={onSignUp} className="luxury-button px-8 sm:px-10 py-3.5 sm:py-4 text-[10px] sm:text-[11px] shadow-[0_0_30px_rgba(197,160,89,0.15)] w-full sm:w-auto">
        ...
        <motion.div
          ...
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] sm:w-[600px] sm:h-[600px] bg-gold/5 blur-[120px] rounded-full pointer-events-none"
        />
      </section>

      {/* Stats Bar */}
      <section className="border-y border-[#1A1A1A] bg-[#070707] py-8 sm:py-12 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto grid grid-cols-2 sm:flex sm:flex-wrap justify-center gap-6 sm:gap-12 lg:gap-24">
          {stats.map((stat, i) => (
            <motion.div
              key={stat.label}
              initial={{ opacity: 0, y: 20 }}
            ...
            >
              ...
              <p className="text-[9px] sm:text-[10px] text-[#888] uppercase tracking-[0.2em] font-bold">{stat.label}</p>
            </motion.div>
          ))}
        </div>
      </section>
      ...
      {/* Problem vs Solution */}
      <section id="features" className="py-16 sm:py-32 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto">
          ...
          >
            <h2 className="text-2xl sm:text-4xl font-sans text-white mb-4 sm:mb-6">Designed for Operation-Focused Realtors</h2>
            <p className="text-sm sm:text-base text-[#A0A0A0]">Realtors care about getting more leads and closing faster. PropAI attacks the manual friction points that hold you back.</p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-8">
            <motion.div
              ...
              className="bg-[#0A0A0A] p-6 sm:p-10 rounded-2xl sm:rounded-3xl border border-red-900/10 shadow-xl"
            >
              ...
              <ul className="space-y-4 sm:space-y-6">
                {[
                  ...
                  <li key={item} className="flex gap-3 sm:gap-4 items-start text-[#888] line-through decoration-red-900/50 text-sm sm:text-base">
            ...
            <motion.div
              ...
              className="bg-[#0A0A0A] p-6 sm:p-10 rounded-2xl sm:rounded-3xl border border-gold/10 shadow-xl"
            >
              ...
              <ul className="space-y-4 sm:space-y-6">
                {[
                  ...
                  <li key={item} className="flex gap-3 sm:gap-4 items-start text-[#F5F5F5] text-sm sm:text-base">
      ...
      {/* Core Features Bento */}
      <section className="py-16 sm:py-32 bg-[#070707] px-4 sm:px-6">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8">
            {features.map((feature, i) => (
              ...
              >
                <div className="w-10 h-10 sm:w-12 sm:h-12 bg-[#111] rounded-xl flex items-center justify-center mb-4 sm:mb-6 border border-[#222] group-hover:border-gold/20 group-hover:bg-gold/5 transition-all">
                  <feature.icon className="w-5 h-5 sm:w-6 sm:h-6 text-[#777] group-hover:text-gold transition-colors" />
                </div>
      ...
      <section className="py-16 sm:py-24 px-4 sm:px-6 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          >
            <h2 className="text-2xl sm:text-4xl lg:text-6xl font-sans text-white leading-tight">Elevate Your Agency with the <span className="text-gold">PropAI Protocol.</span></h2>
            <p className="text-[#888] text-xs max-w-xl mx-auto leading-relaxed uppercase tracking-[0.2em] font-bold">Reclaim your time. Scale your portfolio. Close faster.</p>
            ...
            <div className="flex flex-col items-center gap-6">
              <div className="mt-4 p-5 sm:p-6 bg-[#0A0A0A]/50 backdrop-blur-sm border border-[#1A1A1A] rounded-xl max-w-xl mx-auto relative overflow-hidden">
                <div className="relative z-10">
                  <h3 className="text-base sm:text-lg font-sans text-white mb-2 tracking-tight">Absolute Simplicity</h3>
                  <p className="text-xs sm:text-sm text-[#777] font-sans leading-relaxed">
      ...
      {/* Footer */}
      <footer className="border-t border-[#1A1A1A] py-6 sm:py-8 px-4 sm:px-6 bg-[#050505]">
        <div className="max-w-7xl mx-auto flex flex-col items-center justify-center text-center gap-3 sm:gap-4">
           ...
           <p className="text-[7px] text-[#555] uppercase tracking-[0.4em] font-black">&copy; 2026 PROPAI LTD. INTEL SECURED.</p>
        </div>
      </footer>
    </div>
  );
