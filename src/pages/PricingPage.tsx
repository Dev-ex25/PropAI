...
export default function PricingPage({ onBack, onSignUp, userEmail }: PricingPageProps) {
  ...
  const config = React.useMemo(() => {
    return {
      reference: selectedPlan ? `${selectedPlan.id}-${Date.now()}` : `placeholder-${Date.now()}`,
      email: email || 'customer@example.com',
      amount: selectedPlan ? Math.round(selectedPlan.price * 100) : 100, // Smallest possible amount if none selected
      publicKey: publicKey,
      currency: 'USD',
    };
  }, [selectedPlan, email, publicKey]);
